...
export function useAdminCustomers(params: AdminCustomerQuery = {}) {
  return useQuery({ queryKey: queryKeys.adminCustomers.all(params), queryFn: () => adminCustomerService.getAll(params) });
}

export function useAdminCustomerDetail(customerId: string) {
  return useQuery({
    queryKey: queryKeys.adminCustomers.detail(customerId),
...
export function useAdminCustomerOrders(customerId: string, status?: OrderStatus) {
  return useQuery({
    queryKey: queryKeys.adminCustomers.orders(customerId, status),
...
export function useSetCustomerCod() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => queryClient.invalidateQueries({ queryKey: queryKeys.adminCustomers.detail(vars.customerId) }),
  });
}
