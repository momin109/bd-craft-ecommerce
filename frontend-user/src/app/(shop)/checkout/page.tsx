...
function CheckoutContent() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>(
    user?.codAllowed === false ? "SSLCOMMERZ" : "COD",
  );
    ...
    checkoutMutation.mutate(
      ...
      },
      {
        onSuccess: async (order) => {
          ...
          try {
            const { paymentUrl } =
              await initiatePaymentMutation.mutateAsync(orderId);
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-2">
          Thank you, {form.fullName || "valued customer"}!
        </p>
        ...
        <div className="p-4 bg-brand-50 rounded-xl border border-brand-100 mb-8 text-left space-y-2">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-brand-700">
              {formatPriceBDT(placedOrder.totalPayable)}
            </span>
          </div>
        </div>
        <div className="flex gap-3">
          <Link
            href="/account"
            className="flex-1 py-3 border border-brand-200 text-brand-700 rounded-xl font-medium text-sm hover:bg-brand-50 transition-colors text-center"
          >
            Track Order
          </Link>
          <Link
            href="/"
            className="flex-1 py-3 bg-brand-700 text-white rounded-xl font-semibold text-sm hover:bg-brand-800 transition-colors text-center"
          >
            Continue Shopping
          </Link>
        </div>
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-6 py-3 bg-brand-700 text-white rounded-xl font-semibold hover:bg-brand-800 transition-colors"
        >
          Continue Shopping
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          {step === "address" && (
            <div className="bg-white border border-gray-100 rounded-2xl p-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="e.g. Mirpur Upazila"
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:border-brand-400 text-sm"
                  />
                </div>
                <div className="sm:col-span-2">
              ...
              <button
                ...
                disabled={!form.fullName || !form.mobile || !form.addressLine}
          ...
          {step === "payment" && (
            <div className="bg-white border border-gray-100 rounded-2xl p-6 animate-fade-in">
              <h2 className="font-serif text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                <CreditCard size={20} className="text-brand-600" /> Payment
                Method
              </h2>
                ...
                {PAYMENT_METHODS.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={disabled}
                      className={cn(
                        "w-full flex items-center gap-3 p-4 border-2 rounded-xl transition-all text-left",
                        disabled
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 border border-gray-200 text-gray-600 rounded-xl font-medium hover:bg-gray-50 transition-colors"
                >
                  Back
                </button>
                <button
                  onClick={() => setStep("review")}
                  className="flex-1 py-3.5 bg-brand-700 hover:bg-brand-800 text-white font-semibold rounded-xl transition-all"
                >
                  Review Order
                </button>
              </div>
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="bg-white border border-gray-100 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                    <MapPin size={15} className="text-brand-600" /> Delivery
                    Address
                  </h3>
                  <button
                    onClick={() => setStep("address")}
                ...
                <p className="text-sm text-gray-500">
                  {form.addressLine}, {form.area}, {form.city}, {form.district}
                </p>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xl">
                    {PAYMENT_METHODS.find((m) => m.id === paymentMethod)?.logo}
                  </span>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-5">
                <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
                  <Package size={15} className="text-brand-600" /> Order Items (
                  {items.length})
                </h3>
            ...
            <div className="space-y-2 text-sm pt-3 border-t border-gray-100">
              ...
              {couponFromCart && (
                <div className="flex justify-between text-green-600">
                  <span>Coupon</span>
                  <span>{couponFromCart}</span>
                </div>
              )}
              <div className="flex justify-between text-gray-500">
                <span>Shipping</span>
                <span>{formatPriceBDT(shippingCharge)}</span>
              </div>
              <p className="text-[11px] text-gray-400 pt-1">
                Final discount (coupon/offers) is calculated by the server and
                shown on the confirmation page.
              </p>
              <div className="flex justify-between font-bold text-gray-900 pt-2 border-t border-gray-100 text-base">
                <span>Estimated Total</span>
                <span className="text-brand-700">
                  {formatPriceBDT(subtotal + shippingCharge)}
                </span>
              </div>
            </div>
          </div>
