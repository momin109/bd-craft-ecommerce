...
import { useCart, useUpdateCartItem, useRemoveCartItem, useClearCart } from "@/hooks/queries/useCart";
...
export default function CartPage() {
    ...
    applyCouponMutation.mutate(
      { code: coupon.trim() },
      {
        onSuccess: (res) => {
          ...
          setCouponMessage({ type: "success", text: `Coupon applied! ৳${res.discountAmount} off` });
        },
        onError: (err) => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        ...
        <h1 className="font-serif text-3xl font-bold text-gray-800 mb-3">Sign in to view your bag</h1>
  ...
  if (isError) return <ErrorState message="Couldn't load your cart." onRetry={() => refetch()} />;

  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        <ShoppingBag size={72} className="text-brand-200 mx-auto mb-6" />
        <h1 className="font-serif text-3xl font-bold text-gray-800 mb-3">Your bag is empty</h1>
        <p className="text-gray-400 mb-8">Add some beautiful pieces to get started</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <h1 className="font-serif text-3xl font-bold text-gray-900 mb-8">
        Shopping Bag <span className="text-gray-400 font-normal text-xl">({items.length} items)</span>
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={itemId} className="flex gap-4 p-4 bg-white border border-gray-100 rounded-2xl hover:border-brand-200 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <Link href={`/products/${item.product?.slug}`} className="font-semibold text-gray-800 hover:text-brand-700 transition-colors line-clamp-2">
                        {item.product?.name}
                      </Link>
                      <div className="flex gap-3 mt-1">
                        {item.variant?.size && <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded">Size: {item.variant.size}</span>}
                        {item.variant?.color && (
                          <span className="flex items-center gap-1 text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded">
                            {item.variant.colorCode && <span className="w-2.5 h-2.5 rounded-full border" style={{ background: item.variant.colorCode }} />}
                            {item.variant.color}
                          </span>
                        )}
                      </div>
                    ...
                    <button
                      onClick={() => removeItem.mutate(itemId, { onError: () => toast.error("Couldn't remove item") })}
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                      <button
                        onClick={() => updateItem.mutate({ cartItemId: itemId, quantity: Math.max(1, item.quantity - 1) })}
                      ...
                      <button
                        onClick={() => updateItem.mutate({ cartItemId: itemId, quantity: item.quantity + 1 })}
                        className="w-8 h-8 flex items-center justify-center hover:bg-gray-50 transition-colors"
                      >
                        <Plus size={13} />
                      </button>
                    ...
                    <div className="text-right">
                      <span className="font-bold text-brand-700">{formatPriceBDT(price * item.quantity)}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-end">
            <button
              onClick={() => clearCartMutation.mutate(undefined, { onError: () => toast.error("Couldn't clear cart") })}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 sticky top-24">
            <h2 className="font-semibold text-gray-800 mb-5 pb-3 border-b border-gray-100">Order Summary</h2>
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2">
                <input
                  value={coupon}
                  onChange={(e) => { setCoupon(e.target.value); setCouponMessage(null); }}
              ...
              {couponMessage && (
                <p className={`text-xs mt-1.5 ${couponMessage.type === "success" ? "text-green-600" : "text-red-500"}`}>
                  {couponMessage.text}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2.5 text-sm">
              <div className="flex justify-between text-gray-600"><span>Subtotal ({items.length} items)</span><span>{formatPriceBDT(subtotal)}</span></div>
              {appliedDiscount > 0 && (
                <div className="flex justify-between text-green-600"><span>Coupon Discount ({appliedCode})</span><span>-{formatPriceBDT(appliedDiscount)}</span></div>
              )}
              <div className="flex justify-between text-gray-600">
                <span>Delivery Fee</span>
                <span className={deliveryFee === 0 ? "text-green-600 font-medium" : ""}>{deliveryFee === 0 ? "FREE" : formatPriceBDT(deliveryFee)}</span>
              </div>
              {subtotal < 1500 && (
            ...
            <div className="flex justify-between font-bold text-gray-900 pt-4 mt-3 border-t border-gray-100">
              <span>Total</span>
              <span className="text-brand-700 text-lg">{formatPriceBDT(finalTotal)}</span>
            </div>

            <Link
              href={appliedCode ? `/checkout?coupon=${encodeURIComponent(appliedCode)}` : "/checkout"}
            ...
            <div className="mt-4 flex items-center justify-center gap-4">
              {["bKash", "Nagad", "Rocket", "VISA"].map((p) => (
                <span key={p} className="text-[10px] text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100">{p}</span>
              ))}
            </div>
          </div>
        </div>
